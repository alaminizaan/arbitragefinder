...
def calculate_arbitrage_opportunity(exchange, coin_pair, fees=None):
    if fees is None:
        # fetch fees for the exchange
        try:
            fees = exchange.fetch_fees()
            if 'maker' not in fees['trading']:
                return {'error': f"'maker' key not found in trading fees for {exchange.name}"}
        except Exception as e:
            return {'error': f'Failed to fetch fees on {exchange.name}: {str(e)}'}
...
def get_arbitrage_opportunities():
    ...
    for exchange_data in exchanges:
        ...
        for coin_pair in coins:
            opportunity = calculate_arbitrage_opportunity(exchange, coin_pair)
